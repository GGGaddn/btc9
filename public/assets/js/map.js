jQuery(function($) {
  $('path').hover(function(e){
    ...
    if($(this).attr('name')) {
      var name = $(this).attr('name')  + ' ' + $(this).attr('cnt');
   ...
   */
   ...
   // script.classList.add('change');
    ...
    for (var j = 0; j < idAarr2.length; j++) {
      var regId = idAarr2[j][0];
      $('#' + regId).css('fill', $('#' + regId).attr('color'));
    }
    $(this).css('fill', '#7bd17b');

    //$('path').not(this).css('fill','rgba(0,0,0,0.5)');

    $('.indicator').css({'top':e.pageY - 400,'left':e.pageX - 200}).show();
